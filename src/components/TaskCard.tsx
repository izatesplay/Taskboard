...
interface TaskCardProps {
  ...
  onDropOnTask?: (draggedTaskId: string, targetTaskId: string, dropPosition: 'before' | 'after') => void;
...
export default function TaskCard({ 
  ...
  const [dragOverPosition, setDragOverPosition] = useState<false | 'before' | 'after'>(false);
  ...
  const handleDropCard = (e: React.DragEvent) => {
    ...
    if (draggedTaskId && draggedTaskId !== task.id && onDropOnTask) {
      onDropOnTask(draggedTaskId, task.id, pos);
    }
  };
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4.5 shadow-sm ${style.shadowGlow} transition-shadow duration-300 cursor-grab active:cursor-grabbing ${style.borderCol} ${
        dragOverPosition ? 'ring-2 ring-purple-500/60 bg-purple-500/5 dark:bg-purple-950/20 shadow-md' : ''
      }`}
    >
      {/* Insertion line indicator for vertical dragging */}
      {dragOverPosition === 'before' && (
        <div className="absolute top-0 left-2 right-2 h-1 bg-purple-500 rounded-full z-30 animate-pulse shadow-[0_0_10px_rgba(168,85,247,0.9)]" />
      )}
      {dragOverPosition === 'after' && (
        <div className="absolute bottom-0 left-2 right-2 h-1 bg-purple-500 rounded-full z-30 animate-pulse shadow-[0_0_10px_rgba(168,85,247,0.9)]" />
      )}
      ...
      {/* Card Header with Task Number Badge and Priority Tag */}
      <div className="flex items-center justify-between gap-1.5 mb-2.5 pt-1" style={{ transform: 'translateZ(15px)' }}>
        <div className="flex items-center gap-1.5">
          {index !== undefined && (
            <div 
              className="flex items-center gap-1 bg-purple-100/90 dark:bg-purple-950/80 text-purple-900 dark:text-purple-200 px-2.5 py-0.5 rounded-lg border border-purple-200/80 dark:border-purple-800/60 shadow-3xs font-mono font-bold text-[11px]"
              title={`موقعیت تسک شماره ${index}`}
            >
              <GripVertical className="w-3.5 h-3.5 text-purple-500/80 shrink-0 cursor-grab" />
              <span className="text-[10px] text-purple-600/70 dark:text-purple-400/70">#</span>
        ...
        <div className="text-[10px] text-slate-400 dark:text-slate-500 flex items-center gap-1 font-mono font-semibold">
